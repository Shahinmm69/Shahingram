...
namespace WebFramework.Api
{
    //[ApiVersion("1")]
    [AllowAnonymous]
    public class CrudControllerType3<TDto, TSelectDto, TEntity> : BaseController
        where TDto : BaseDto<TDto, TEntity>, new()
        where TSelectDto : BaseSelectDto<TSelectDto, TEntity>, new()
        where TEntity : Modification, new()
    {
        private readonly IRepository<TEntity> _repository;
        private readonly IModificationRepository<TEntity> modificationRepository;
        private readonly SignInManager<User> signInManager;

        public CrudControllerType3(IRepository<TEntity> repository, IModificationRepository<TEntity> modificationRepository, SignInManager<User> signInManager)
        {
